...
import {
  ContractTransactionResponse as RawContractTransactionResponse,
...
import { ContractTransaction } from '@wavesenterprise/we-node-grpc-api'


describe('TransactionConverter', () => {
  let mockTx = RawContractTransactionResponse.fromPartial({
  ...
  describe('parce incoming tx', () => {
    ...
    beforeAll(() => {
      tx = convertContractTransaction(mockTx.transaction as ContractTransaction)
    })

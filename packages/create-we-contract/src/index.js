...
prog
    ...
    .action(async (contractName, opts) => {
...
Try ${chalk.yellowBright('we-create-contract MyContract')}
    ...
    try {
        ...
        await fs.copy(path.resolve(path.dirname('.'), `./template`), projectPath, {
            overwrite: true,
        });
        await fs.move(path.resolve(projectPath, './_gitignore'), path.resolve(projectPath, './.gitignore'), { overwrite: true });
        await fs.move(path.resolve(projectPath, './src/contract'), contractPath, { overwrite: true });
        let contractTpl = await fs.readFile(contractPath, { encoding: 'utf-8' });
        ...
        let entrypoint = await fs.readFile(path.resolve(projectPath, './index.ts'), { encoding: 'utf-8' });
        entrypoint = entrypoint.replace('./src/contract', relContractPath);
        await fs.writeFile(path.resolve(projectPath, './index.ts'), entrypoint, {
            encoding: 'utf-8',
        });
        bootSpinner.succeed();
    }
